...
public class UserAdapter extends RecyclerView.Adapter<UserAdapter.CustomViewHolder> {
    ...
    public UserAdapter(Context context, ArrayList<Profile> list) {
        mList = list;
        mContext = context;
    }
    ...
    public void onBindViewHolder(final CustomViewHolder holder, int position) {
        Profile model = mList.get(position);
...
//                            Profile model = mList.get(getAdapterPosition());
//                            FirebaseDatabase.getInstance().getReference("blocks").child(cUId).child(model.getId())
//                                    .setValue(1);
...
//                } else
                    ...
                    i.putExtra(Constants.EXTRA_USERID, mList.get(getAdapterPosition()).getId());
